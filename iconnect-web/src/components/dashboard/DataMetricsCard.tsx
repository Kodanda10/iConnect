...
export default function DataMetricsCard() {
    ...
    // Optimize: Stabilized event handlers with useCallback to prevent recreating
    // anonymous inline functions on every render cycle, which previously bypassed React.memo.
                        ...
                        {gpLoading[hoveredBlock] ? (
                            ...
                            <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                                {(() => {
                                    ...
                                    // Optimize: Extracted max calculation OUTSIDE the .map loop.
                                    // Prevents an O(N^2) operation where Math.max was previously
                                    // recalculated across the entire array for every single list item.
                                    const maxCount = Math.max(...gps.map(g => g.count), 1);
                                    return gps.map((gp, index) => (
                                        <GPProgressBar
                                            key={gp.name}
                                            gp={gp}
                                            maxCount={maxCount}
