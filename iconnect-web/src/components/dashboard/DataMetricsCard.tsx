 ...
 * - 2025-12-18: Performance optimization - Memoized BlockItem and stabilized callbacks to prevent list re-renders (Bolt)
 */
...
export default function DataMetricsCard() {
    ...
    // Ref to track loaded blocks to avoid dependency on gpData/gpLoading in useCallback
    ...
    // Lazy load GP data on hover - Stable callback
    const loadGPData = useCallback(async (blockName: string) => {
        // Use ref to check if already loaded/loading to avoid state dependencies
        if (loadedBlocksRef.current.has(blockName)) return;
        ...
        try {
            ...
            // On error, remove from loaded set so we can retry?
            // For now, keep it to avoid infinite error loops on hover
        } finally {
...
interface BlockItemProps {
    ...
    total: number;
    isHovered: boolean;
    onBlockHover: (name: string) => void;
    onBlockLeave: () => void;
}

const BlockItem = React.memo(({ block, total, isHovered, onBlockHover, onBlockLeave }: BlockItemProps) => {
    ...
    return (
        <div
            ...
            `}
            onMouseEnter={() => onBlockHover(block.name)}
