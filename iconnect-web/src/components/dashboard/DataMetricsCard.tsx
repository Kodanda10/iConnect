 ...
 * - [Current Date]: BOLT OPTIMIZATION - Extracted Math.max calculation from O(N²) loop to O(N). Added React.memo and stable callbacks to prevent unnecessary BlockItem re-renders.
 */
                ...
                {hoveredBlock ? (
                    ...
                    <div className="animate-fade-in space-y-4">
                        ...
                        {gpLoading[hoveredBlock] ? (
                            ...
                            // ⚡ BOLT OPTIMIZATION: Extract Math.max computation out of the render loop
                            // Previously O(N²) because Math.max was calculated inside the .map loop for each GP item
                            // Now O(N) by calculating maxCount once outside the loop
                            const currentGpData = gpData[hoveredBlock] || [];
                            const maxGpCount = currentGpData.length > 0
                                ? Math.max(...currentGpData.map(g => g.count))
                                : 1;
