 ...
 * - 2025-01-11: Performance optimization: Memoized BlockItem and stabilized handlers to prevent O(N) re-renders on hover
 */
...
export default function DataMetricsCard() {
    ...
    // Performance: Use ref to track cache to avoid re-creating loadGPData on every render
    const gpCache = useRef<Set<string>>(new Set());
    ...
    // Performance: Stable callback that doesn't depend on state (uses Ref for cache check)
    const loadGPData = useCallback(async (blockName: string) => {
        if (gpCache.current.has(blockName)) return;

        gpCache.current.add(blockName);
        ...
        try {
            ...
            gpCache.current.delete(blockName); // Allow retry on error
        } finally {
            setGpLoading(prev => ({ ...prev, [blockName]: false }));
        }
    }, []);

    // Performance: Memoized handlers to pass to memoized BlockItem
...
// Performance: Memoize BlockItem to prevent re-renders of non-hovered items
const MemoizedBlockItem = memo(function BlockItem({ block, total, isHovered, onHover, onLeave }: BlockItemProps) {
