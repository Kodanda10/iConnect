        ...
        <div className={`bg-zinc-900 border border-white/10 shadow-2xl p-4 rounded-xl ${className}`} onClick={handleCalendarClick}>
            ...
            <div className="flex items-center justify-between mb-4">
                <button
                    ...
                    className="p-2 rounded-lg hover:bg-white/10 text-white/60 hover:text-white focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:outline-none transition-colors"
                ...
                <div className="flex items-center gap-1">
                    ...
                    <div className="relative dropdown-container">
                        <button
                            ...
                            className="px-2 py-1 rounded-lg hover:bg-white/10 text-sm font-bold text-white flex items-center gap-1 focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:outline-none transition-colors"
                            ...
                            <div className="absolute top-full left-0 mt-1 bg-zinc-900/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl z-50 p-2 grid grid-cols-3 gap-1 w-40" role="listbox">
                                {monthNamesShort.map((month, index) => (
                                    <button
                                        ...
                                        className={`px-2 py-1.5 text-xs rounded-lg focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:outline-none transition-colors ${viewDate.getMonth() === index
                            ...
                            <div className="absolute top-full right-0 mt-1 bg-zinc-900/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl z-50 p-2 max-h-48 overflow-y-auto w-24 scrollbar-thin scrollbar-thumb-white/20" role="listbox">
                                {years.map(year => (
                                    <button
                                        ...
                                        className={`w-full px-2 py-1.5 text-xs rounded-lg focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:outline-none transition-colors text-center ${viewDate.getFullYear() === year
                                            ? 'bg-emerald-500 text-white'
                                            : 'text-white/70 hover:bg-white/10 hover:text-white'
                                            }`}
                ...
                {Array.from({ length: daysInMonth }, (_, i) => {
                    ...
                    return (
                        <button
                            ...
                            className={`
                                aspect-square flex items-center justify-center text-xs font-medium focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-emerald-400 focus-visible:outline-none transition-all relative
                            ...
                            aria-label={`${date.toDateString()}${isTodayDay ? ' (Today)' : ''}${hasEvent ? ' (Has Event)' : ''}`}
                        >
