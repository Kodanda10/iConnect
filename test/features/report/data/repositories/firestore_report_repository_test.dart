...
void main() {
  ...
  setUp(() {
    ...
    when(
      () => mockFirestore.collection('action_logs'),
    ).thenReturn(mockActionLogs);
  ...
  group('getReportForDays', () {
    test('returns list of DaySummary grouped by date', () async {
      ...
      when(() => mockDoc2.data()).thenReturn({
        'id': '2',
        'action_type': 'SMS',
        'executed_at': Timestamp.fromDate(
      ...
      when(
        () => mockQuery.orderBy(any()),
      ).thenReturn(mockQuery); // assuming we order by date
      ...
      // Assert
      result.fold((l) => fail('Returned Failure: ${l.message}'), (summaries) {
