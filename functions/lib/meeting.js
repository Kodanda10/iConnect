...
exports.createMeetingTicker = (0, https_1.onCall)({ region: "asia-south1" }, async (request) => {
    ...
    try {
        ...
        if (meetingType === 'VIDEO_MEET') {
            ...
            if (data.targetBlock)
                tickerData.targetBlock = data.targetBlock;
            if (data.targetGP)
                tickerData.targetGP = data.targetGP;
        }
        ...
        if (meetingType === 'CONFERENCE_CALL') {
            const { queryConstituentsByAudience, sendBulkSMS } = await Promise.resolve().then(() => __importStar(require('./audienceQuery')));
            const { formatAudioMessage, schedulePushForLeader } = await Promise.resolve().then(() => __importStar(require('./notifications')));
            ...
            const constituents = await queryConstituentsByAudience(targetAudience, targetBlock, targetGP);
