...
const generateTasksForDateRange_1 = require("../generateTasksForDateRange");
// Mock Firestore Timestamp
const mockTimestamp = {
    fromDate: (date) => ({ toDate: () => date, _seconds: Math.floor(date.getTime() / 1000) }),
    now: () => ({ toDate: () => new Date(), _seconds: Math.floor(Date.now() / 1000) }),
};
// Helper to create constituent
const createConstituent = (id, name, dobMonth, dobDay, anniversaryMonth, anniversaryDay) => ({
    id,
    name,
...
describe('generateTasksForDateRange', () => {
    ...
    describe('Idempotency', () => {
        it('does not create duplicate tasks when run twice', () => {
            ...
            // First run
            const result1 = (0, generateTasksForDateRange_1.generateTasksForDateRange)(constituents, [], options);
            expect(result1.newTasks).toHaveLength(1);
            // Second run with existing tasks from first run
            const result2 = (0, generateTasksForDateRange_1.generateTasksForDateRange)(constituents, result1.newTasks, options);
        ...
        it('tracks skipped duplicates count', () => {
            ...
            const result = (0, generateTasksForDateRange_1.generateTasksForDateRange)(constituents, existingTasks, options);
    ...
    describe('Error Handling', () => {
        it('handles constituents with missing DOB gracefully', () => {
            const constituents = [
                ...
                { id: 'c2', name: 'No DOB Person', mobile_number: '123' },
            ];
            const options = {
                startDate: new Date(2025, 11, 25),
                endDate: new Date(2025, 11, 25),
        ...
        it('handles invalid date formats gracefully', () => {
            const constituents = [
                { id: 'c1', name: 'Bad Date', dob: 'not-a-date', mobile_number: '123' },
            ];
            const options = {
                startDate: new Date(2025, 11, 25),
                endDate: new Date(2025, 11, 25),
            ...
            // Should not throw
            expect(() => (0, generateTasksForDateRange_1.generateTasksForDateRange)(constituents, [], options)).not.toThrow();
        });
    });
...
//# sourceMappingURL=generateTasksForDateRange.test.js.map