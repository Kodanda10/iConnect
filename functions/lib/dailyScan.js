...
function scanForTasks(constituents, existingTasks, TimestampClass) {
    ...
    // Extract static dates to prevent repetitive and expensive .toISOString calls inside the loop
    const todayStr = today.toISOString().split('T')[0];
    const tomorrowStr = tomorrow.toISOString().split('T')[0];
    // Pre-compute O(1) lookup set for existing task signatures instead of O(n^2) nested search
    ...
    for (const task of existingTasks) {
        ...
        const constId = task.constituent_id || task.constituentId;
        existingTaskSignatures.add(`${constId}_${task.type}_${taskDateStr}`);
    }
    const newTasks = [];
    for (const constituent of constituents) {
        // Check Birthday - Today
        if (isDateMatch(constituent.dob, today)) {
            if (!existingTaskSignatures.has(`${constituent.id}_BIRTHDAY_${todayStr}`)) {
        ...
        if (isDateMatch(constituent.dob, tomorrow)) {
            if (!existingTaskSignatures.has(`${constituent.id}_BIRTHDAY_${tomorrowStr}`)) {
        ...
        if (isDateMatch(constituent.anniversary, today)) {
            if (!existingTaskSignatures.has(`${constituent.id}_ANNIVERSARY_${todayStr}`)) {
        ...
        if (isDateMatch(constituent.anniversary, tomorrow)) {
            if (!existingTaskSignatures.has(`${constituent.id}_ANNIVERSARY_${tomorrowStr}`)) {
