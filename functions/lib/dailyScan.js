...
function scanForTasks(constituents, existingTasks, TimestampClass) {
    ...
    // Pre-calculate date strings once outside the loop
    const todayStr = today.toISOString().split('T')[0];
    const tomorrowStr = tomorrow.toISOString().split('T')[0];
    // Pre-compute O(1) lookup set for existing tasks
    ...
    for (const task of existingTasks) {
        ...
        existingKeys.add(`${constituentId}_${task.type}_${taskDateStr}`);
    }
    const newTasks = [];
    for (const constituent of constituents) {
        // Check Birthday - Today
        if (isDateMatch(constituent.dob, today)) {
            if (!existingKeys.has(`${constituent.id}_BIRTHDAY_${todayStr}`)) {
        ...
        if (isDateMatch(constituent.dob, tomorrow)) {
            if (!existingKeys.has(`${constituent.id}_BIRTHDAY_${tomorrowStr}`)) {
        ...
        if (isDateMatch(constituent.anniversary, today)) {
            if (!existingKeys.has(`${constituent.id}_ANNIVERSARY_${todayStr}`)) {
        ...
        if (isDateMatch(constituent.anniversary, tomorrow)) {
            if (!existingKeys.has(`${constituent.id}_ANNIVERSARY_${tomorrowStr}`)) {
