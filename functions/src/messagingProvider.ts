...
import { redactMessage, redactMobile } from './utils/security';
 ...
 */
export function createMessagingProvider(providerType?: string): MessagingProvider {
...
class MockProvider implements MessagingProvider {
    ...
    async sendWhatsApp(mobile: string, template: string, params: Record<string, string>): Promise<SMSResult> {
        console.log(`[MOCK WhatsApp] To: ${redactMobile(mobile)} | Template: ${template} | Params: [REDACTED_PARAMS]`);
        return {
            success: true,
    ...
    async sendBulkSMS(mobiles: string[], message: string): Promise<SMSResult[]> {
 ...
 */
class MSG91Provider implements MessagingProvider {
    name = 'MSG91Provider';
