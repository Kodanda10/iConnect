...
import { handleMeetingCreated } from '../triggers';
import { sendPushNotification } from '../messaging';
import { determinePushTimes } from '../notifications';
...
describe('Triggers', () => {
    describe('handleMeetingCreated', () => {
        it('calculates notification times and schedules tasks', async () => {
            ...
            await handleMeetingCreated(mockMeeting);

            // Should verify times were calculated
            expect(determinePushTimes).toHaveBeenCalled();
