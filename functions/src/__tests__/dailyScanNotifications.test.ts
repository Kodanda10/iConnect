...
describe('scheduleDailyNotifications', () => {
    ...
    test('should schedule Action (Tomorrow Morning) and Heads Up (Tonight) with correct dynamic text', async () => {
        // Set Today to Dec 18, 2025
        const today = new Date('2025-12-18T10:00:00Z');
        jest.setSystemTime(today);

        const constituents: Constituent[] = [
            // Birthday Today (Dec 18) - ignored for tomorrow's notifications
        ...
        // 1 Birthday Tomorrow (displayed as "Today" in the morning notification)
        ...
        // Check Heads Up Notification (For Tonight)
    ...
    test('should NOT schedule Action if no events tomorrow', async () => {
        ...
        // No events tomorrow (Dec 19), so no notifications
        const actionCall = mockBatch.set.mock.calls.find(c => c[1].type === 'ACTION_REMINDER');
