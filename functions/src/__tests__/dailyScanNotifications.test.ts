...
describe('scheduleDailyNotifications', () => {
    ...
    test('should schedule Action (Today) and Heads Up (Tomorrow) with correct dynamic text', async () => {
        ...
        // 1 Birthday Today (C) - Action Reminder targets Tomorrow's events but says "today"
        ...
        // Check Time: Tomorrow 8:00 AM (Action Reminder is scheduled for tomorrow morning)
    ...
    test('should NOT schedule Action if no events tomorrow', async () => {
        ...
        const constituents: Constituent[] = [
            // Birthday Today (Dec 18) - Should be ignored by Action Reminder (which looks for Tomorrow)
            { id: '1', name: 'A', dob: '1990-12-18', mobile_number: '', ward_number: '', address: '', created_at: '' }
        ];
        ...
        const headsUpCall = mockBatch.set.mock.calls.find(c => c[1].type === 'HEADS_UP');
        // Heads Up also looks for Tomorrow. If only Today events exist, no Heads Up?
        // Logic: if (headsUpEnabled && (tomorrowCount > 0))
        // So Heads Up should also be Undefined if no tomorrow events.
