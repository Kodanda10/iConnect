 ...
 * using a pre-computed Set for O(1) lookups
 */
function taskExists(
    taskSet: Set<string>,
    ...
    return taskSet.has(`${constituentId}_${type}_${dueDate}`);
}
...
export function scanForTasks(
    ...
    // Performance Optimization: Pre-compute task signatures into a Set for O(1) lookups
    ...
    for (const constituent of constituents) {
        ...
        if (isDateMatch(constituent.dob, today)) {
            ...
            if (!taskExists(existingTaskSet, constituent.id, 'BIRTHDAY', dueDateStr)) {
        ...
        if (isDateMatch(constituent.anniversary, today)) {
            const dueDateStr = today.toISOString().split('T')[0];
            if (!taskExists(existingTaskSet, constituent.id, 'ANNIVERSARY', dueDateStr)) {
