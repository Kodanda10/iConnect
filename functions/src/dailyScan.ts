 ...
 */
function taskExists(
    existingTaskSet: Set<string>,
    constituentId: string,
    type: TaskType,
    dueDate: string
): boolean {
    return existingTaskSet.has(`${constituentId}_${type}_${dueDate}`);
}
...
export function scanForTasks(
    ...
    // Pre-allocate ISO strings to avoid string creation in hot loop
    const todayStr = today.toISOString().split('T')[0];
    const tomorrowStr = tomorrow.toISOString().split('T')[0];

    // Build O(1) lookup set for existing tasks
    ...
    for (const constituent of constituents) {
        ...
        if (isDateMatch(constituent.dob, today)) {
            if (!taskExists(existingTaskSet, constituent.id, 'BIRTHDAY', todayStr)) {
        ...
        if (isDateMatch(constituent.dob, tomorrow)) {
            if (!taskExists(existingTaskSet, constituent.id, 'BIRTHDAY', tomorrowStr)) {
        ...
        if (isDateMatch(constituent.anniversary, today)) {
            if (!taskExists(existingTaskSet, constituent.id, 'ANNIVERSARY', todayStr)) {
        ...
        if (isDateMatch(constituent.anniversary, tomorrow)) {
            if (!taskExists(existingTaskSet, constituent.id, 'ANNIVERSARY', tomorrowStr)) {
