...
export function scanForTasks(
    ...
    // Hoist date string formatting outside the loop to avoid repeated allocations
    const todayStr = today.toISOString().split('T')[0];
    const tomorrowStr = tomorrow.toISOString().split('T')[0];

    // Pre-construct a Set for O(1) lookups instead of O(N^2) nested array .some() loops
    ...
    for (const task of existingTasks) {
        ...
        taskSet.add(`${cId}_${task.type}_${taskDateStr}`);
    }
