 ...
 * - 2024-05-20: Optimized isDateMatch with fast-path string comparison
 */
...
/**
 * Generate -MM-DD suffix for fast date matching
 */
...
function isDateMatch(dateStr: string | undefined, targetDate: Date, suffix?: string): boolean {
    ...
    // Fast path: exact string match for standard YYYY-MM-DD
    if (suffix && dateStr.length === 10 && dateStr.endsWith(suffix)) {
        return true;
    }

    // Handle YYYY-MM-DD format (Slow path)
