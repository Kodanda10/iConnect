 ...
 */
function isDateMatch(dateStr: string | undefined, targetDate: Date, fastSuffix?: string): boolean {
    if (!dateStr) return false;

    // Optimization: For standard YYYY-MM-DD dates, use string comparison
    // This avoids split/parseInt for the vast majority of non-matches
    if (fastSuffix && dateStr.length === 10 && dateStr.charAt(4) === '-') {
        return dateStr.endsWith(fastSuffix);
    }
