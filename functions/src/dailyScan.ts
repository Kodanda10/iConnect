...
export function scanForTasks(
    ...
    // ⚡ Bolt Optimization: Extract expensive string formatting outside the hot loop
    const todayStr = today.toISOString().split('T')[0];
    const tomorrowStr = tomorrow.toISOString().split('T')[0];

    // ⚡ Bolt Optimization: O(1) Set lookup instead of O(N^2) array searches
    const existingTasksSet = new Set<string>();
    for (const task of existingTasks) {
        let taskDateStr = '';
        if (task.due_date && typeof (task.due_date as any).toDate === 'function') {
            taskDateStr = (task.due_date as any).toDate().toISOString().split('T')[0];
        ...
        if (constituentId && task.type && taskDateStr) {
            existingTasksSet.add(`${constituentId}_${task.type}_${taskDateStr}`);
        }
    }
    ...
    for (const constituent of constituents) {
        ...
        if (isDateMatch(constituent.dob, today)) {
            if (!existingTasksSet.has(`${constituent.id}_BIRTHDAY_${todayStr}`)) {
        ...
        if (isDateMatch(constituent.dob, tomorrow)) {
            if (!existingTasksSet.has(`${constituent.id}_BIRTHDAY_${tomorrowStr}`)) {
        ...
        if (isDateMatch(constituent.anniversary, today)) {
            if (!existingTasksSet.has(`${constituent.id}_ANNIVERSARY_${todayStr}`)) {
        ...
        if (isDateMatch(constituent.anniversary, tomorrow)) {
            if (!existingTasksSet.has(`${constituent.id}_ANNIVERSARY_${tomorrowStr}`)) {
