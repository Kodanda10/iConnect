...
export function scanForTasks(
    ...
    // Pre-calculate Set of existing tasks for O(1) lookups
    ...
    for (const task of existingTasks) {
        ...
        taskSet.add(`${constituentId}_${task.type}_${taskDateStr}`);
    }

    const newTasks: Task[] = [];

    // ⚡ Bolt: Use pre-calculated O(1) Set lookup and pre-formatted date strings instead of
    // an O(N^2) nested search with repeated date formatting to improve performance.
