 ...
 * - 2025-01-28: Optimized date matching performance
 */
...
/**
 * Helper to get date suffix "-MM-DD" from a Date object
 */
 ...
 * Optimized to use string suffix comparison for standard formats
 */
function isDateMatch(dateStr: string | undefined, suffix: string, targetDate: Date): boolean {
    ...
    // Avoids expensive split/parseInt operations
    if (dateStr.length === 10) {
        return dateStr.endsWith(suffix);
    }

    // Slow Path: Handle non-standard formats (fallback)
