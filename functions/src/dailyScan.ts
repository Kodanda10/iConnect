...
/**
 * Helper to create -MM-DD suffix for fast matching
 */
function toSuffix(date: Date): string {
 ...
 * Optimized with optional suffix for fast string matching
 */
function isDateMatch(dateStr: string | undefined, targetDate: Date, suffix?: string): boolean {
    if (!dateStr) return false;

    // Fast path: use string matching if suffix provided and format looks standard
    if (suffix && dateStr.length === 10 && dateStr[4] === '-') {
