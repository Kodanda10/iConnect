...
/**
 * Helper to get date suffix for fast matching (e.g. "-12-25")
 */
 ...
 * Uses optional suffix for O(1) matching if format is standard YYYY-MM-DD
 */
function isDateMatch(dateStr: string | undefined, targetDate: Date, suffix?: string): boolean {
    if (!dateStr) return false;

    // Fast path: String comparison
    // If dateStr is standard length (10) and suffix is provided, check ending
    if (suffix && dateStr.length === 10 && dateStr.endsWith(suffix)) {
        return true;
    }

    // Slow path: Handle non-standard formats or fallback
