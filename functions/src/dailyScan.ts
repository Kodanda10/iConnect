...
export function scanForTasks(
    ...
    // Pre-compute formatted date strings for today and tomorrow to avoid repeated formatting in the loop
    const todayDateStr = today.toISOString().split('T')[0];
    const tomorrowDateStr = tomorrow.toISOString().split('T')[0];

    // Pre-compute existing task keys into a Set for O(1) lookups instead of O(N) array searching
    ...
    for (const task of existingTasks) {
        ...
        existingTaskKeys.add(`${constituentId}_${task.type}_${taskDateStr}`);
    }
