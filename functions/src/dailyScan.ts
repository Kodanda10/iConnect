...
export function scanForTasks(
    ...
    // Hoist date string formatting outside the loop to avoid redundant allocations
    const todayStr = today.toISOString().split('T')[0];
    const tomorrowStr = tomorrow.toISOString().split('T')[0];

    // Pre-compute a Set of existing task signatures for O(1) lookups instead of O(N^2) array searches
