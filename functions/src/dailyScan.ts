 ...
 */
function isDateMatch(dateStr: string | undefined, targetMonthDay: string): boolean {
    if (!dateStr || dateStr.length !== 10) return false;

    // Fast path: substring comparison to avoid array allocation
    return dateStr.substring(5) === targetMonthDay;
}
...
export function scanForTasks(
    ...
    for (const constituent of constituents) {
        ...
        if (isDateMatch(constituent.dob, todayStr)) {
        ...
        if (isDateMatch(constituent.dob, tomorrowStr)) {
        ...
        if (isDateMatch(constituent.anniversary, todayStr)) {
            const dueDateStr = today.toISOString().split('T')[0];
        ...
        if (isDateMatch(constituent.anniversary, tomorrowStr)) {
            const dueDateStr = tomorrow.toISOString().split('T')[0];
            if (!taskExists(existingTasks, constituent.id, 'ANNIVERSARY', dueDateStr)) {
...
export async function scheduleDailyNotifications(
    ...
    for (const c of constituents) {
        if (isDateMatch(c.dob, todayStr)) todayCount.birthdays++;
        if (isDateMatch(c.anniversary, todayStr)) todayCount.anniversaries++;

        if (isDateMatch(c.dob, tomorrowStr)) tomorrowCount.birthdays++;
        if (isDateMatch(c.anniversary, tomorrowStr)) tomorrowCount.anniversaries++;
    }
    ...
    if (actionEnabled && (tomorrowCount.birthdays + tomorrowCount.anniversaries > 0)) {
        ...
        constituents.forEach(c => {
            if (isDateMatch(c.dob, tomorrowStr) || isDateMatch(c.anniversary, tomorrowStr)) {
