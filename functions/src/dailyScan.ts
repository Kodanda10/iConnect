...
export function scanForTasks(
    ...
    // Pre-calculate ISO strings to avoid string allocation in the hot loop
    const todayStr = today.toISOString().split('T')[0];
    const tomorrowStr = tomorrow.toISOString().split('T')[0];

    // Pre-compute a Set of existing task keys for O(1) lookups
    ...
    for (const task of existingTasks) {
        ...
        if (constituentId && task.type && taskDateStr) {
            existingTaskKeys.add(`${constituentId}-${task.type}-${taskDateStr}`);
        }
    }
    ...
    for (const constituent of constituents) {
        ...
        if (isDateMatch(constituent.dob, today)) {
            if (!existingTaskKeys.has(`${constituent.id}-BIRTHDAY-${todayStr}`)) {
        ...
        if (isDateMatch(constituent.dob, tomorrow)) {
            if (!existingTaskKeys.has(`${constituent.id}-BIRTHDAY-${tomorrowStr}`)) {
        ...
        if (isDateMatch(constituent.anniversary, today)) {
            if (!existingTaskKeys.has(`${constituent.id}-ANNIVERSARY-${todayStr}`)) {
        ...
        if (isDateMatch(constituent.anniversary, tomorrow)) {
            if (!existingTaskKeys.has(`${constituent.id}-ANNIVERSARY-${tomorrowStr}`)) {
