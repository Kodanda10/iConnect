 ...
 * OPTIMIZED: Uses string suffix check first, falls back to parsing for robustness
 */
function isDateMatch(dateStr: string | undefined, targetDate: Date, suffix: string): boolean {
    if (!dateStr) return false;

    // Fast path: String suffix match (avoid parsing)
    if (dateStr.endsWith(suffix)) return true;

    // Slow path: Handle non-padded or malformed YYYY-MM-DD format
...
/**
 * Helper to generate "-MM-DD" suffix from a Date object
 */
function getDobSuffix(date: Date): string {
...
export function scanForTasks(
    ...
    const todaySuffix = getDobSuffix(today);
    const tomorrowSuffix = getDobSuffix(tomorrow);
