...
function taskExists(
    ...
    const key = `${constituentId}_${type}_${dueDate}`;
...
export function scanForTasks(
    ...
    // Pre-compute existing task keys for O(1) lookup
    ...
    for (const constituent of constituents) {
        ...
        if (isDateMatch(constituent.dob, today)) {
            ...
            if (!taskExists(existingTaskKeys, constituent.id, 'BIRTHDAY', dueDateStr)) {
        ...
        if (isDateMatch(constituent.anniversary, today)) {
            const dueDateStr = today.toISOString().split('T')[0];
            if (!taskExists(existingTaskKeys, constituent.id, 'ANNIVERSARY', dueDateStr)) {
