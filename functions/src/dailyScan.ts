...
export function scanForTasks(
    ...
    for (const task of existingTasks) {
        ...
        existingTaskSignatures.add(`${cId}-${task.type}-${taskDateStr}`);
    }
    ...
    for (const constituent of constituents) {
        ...
        if (isDateMatch(constituent.dob, today)) {
            if (!existingTaskSignatures.has(`${constituent.id}-BIRTHDAY-${todayStr}`)) {
        ...
        if (isDateMatch(constituent.dob, tomorrow)) {
            if (!existingTaskSignatures.has(`${constituent.id}-BIRTHDAY-${tomorrowStr}`)) {
        ...
        if (isDateMatch(constituent.anniversary, today)) {
            if (!existingTaskSignatures.has(`${constituent.id}-ANNIVERSARY-${todayStr}`)) {
        ...
        if (isDateMatch(constituent.anniversary, tomorrow)) {
            if (!existingTaskSignatures.has(`${constituent.id}-ANNIVERSARY-${tomorrowStr}`)) {
