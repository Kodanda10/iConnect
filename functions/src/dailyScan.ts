...
export function scanForTasks(
    ...
    // Bolt Optimization: Extract expensive string operations from hot loop
    const todayDateStr = today.toISOString().split('T')[0];
    const tomorrowDateStr = tomorrow.toISOString().split('T')[0];

    // Bolt Optimization: Replace O(N^2) task lookup with O(1) Set
    ...
    for (const constituent of constituents) {
        ...
        if (isDateMatch(constituent.dob, today)) {
            if (!taskSet.has(`${constituent.id}_BIRTHDAY_${todayDateStr}`)) {
        ...
        if (isDateMatch(constituent.dob, tomorrow)) {
            if (!taskSet.has(`${constituent.id}_BIRTHDAY_${tomorrowDateStr}`)) {
        ...
        if (isDateMatch(constituent.anniversary, today)) {
            if (!taskSet.has(`${constituent.id}_ANNIVERSARY_${todayDateStr}`)) {
        ...
        if (isDateMatch(constituent.anniversary, tomorrow)) {
            if (!taskSet.has(`${constituent.id}_ANNIVERSARY_${tomorrowDateStr}`)) {
