...
export function scanForTasks(
    ...
    // Pre-calculate date strings outside the loop to avoid redundant allocations
    const todayStr = today.toISOString().split('T')[0];
    const tomorrowStr = tomorrow.toISOString().split('T')[0];

    // Pre-compute existing tasks into a Set for O(1) lookups instead of O(N^2) array searching
