...
export function scanForTasks(
    ...
    // Bolt ⚡: Extract redundant toISOString calculations outside the hot loop
    const todayDateStr = today.toISOString().split('T')[0];
    const tomorrowDateStr = tomorrow.toISOString().split('T')[0];

    // Bolt ⚡: Pre-compute a Set for O(1) task existence lookup instead of O(N) array searching inside the constituent loop
    ...
    for (const constituent of constituents) {
        ...
        if (isDateMatch(constituent.dob, today)) {
            if (!existingTaskSet.has(`${constituent.id}_BIRTHDAY_${todayDateStr}`)) {
                ...
                existingTaskSet.add(`${constituent.id}_BIRTHDAY_${todayDateStr}`);
            }
        }
        ...
        if (isDateMatch(constituent.dob, tomorrow)) {
            if (!existingTaskSet.has(`${constituent.id}_BIRTHDAY_${tomorrowDateStr}`)) {
                ...
                existingTaskSet.add(`${constituent.id}_BIRTHDAY_${tomorrowDateStr}`);
            }
        }
        ...
        if (isDateMatch(constituent.anniversary, today)) {
            if (!existingTaskSet.has(`${constituent.id}_ANNIVERSARY_${todayDateStr}`)) {
                ...
                existingTaskSet.add(`${constituent.id}_ANNIVERSARY_${todayDateStr}`);
            }
        }
        ...
        if (isDateMatch(constituent.anniversary, tomorrow)) {
            if (!existingTaskSet.has(`${constituent.id}_ANNIVERSARY_${tomorrowDateStr}`)) {
                ...
                existingTaskSet.add(`${constituent.id}_ANNIVERSARY_${tomorrowDateStr}`);
            }
        }
    }
