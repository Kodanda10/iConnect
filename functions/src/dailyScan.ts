...
export function scanForTasks(
    ...
    // ⚡ Bolt: Pre-compute formatted dates outside loop to avoid redundant allocations
    const todayDateStr = today.toISOString().split('T')[0];
    const tomorrowDateStr = tomorrow.toISOString().split('T')[0];

    // ⚡ Bolt: Pre-compute existing task composite keys for O(1) lookup
    // Key format: `${constituentId}_${type}_${dateStr}`
    ...
    for (const task of existingTasks) {
        ...
        if (taskDateStr) {
            existingTaskKeys.add(`${id}_${task.type}_${taskDateStr}`);
        }
    }
