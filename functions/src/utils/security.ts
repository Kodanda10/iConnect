...
/**
 * Sanitizes input to prevent XSS, HTML injection, and mitigate prompt injection.
 * Restricts length to 100 characters and aggressively removes angle brackets (< >) and control characters.
 */
export function sanitizeInput(input: string | null | undefined): string {
    if (!input) return '';
    return input.slice(0, 100).replace(/[<>]/g, '').replace(/[\x00-\x1F\x7F]/g, '').trim();
}
