...
export function sanitizeInput(input: string | null | undefined): string {
    if (!input) return '';
    // Remove control characters (except common whitespace like tab, newline, carriage return)
    // and remove anything resembling HTML/XML tags.
    const noTags = input.replace(/<[^>]*>?/gm, '');
    const sanitized = noTags.replace(/[\x00-\x08\x0B-\x0C\x0E-\x1F\x7F]/g, '');
