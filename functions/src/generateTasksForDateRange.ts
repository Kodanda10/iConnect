...
function parseDateParts(dateStr: string | undefined): { month: number; day: number } | null {
    ...
    // Fast path for YYYY-MM-DD to avoid array allocation in hot loops
    // (10 chars, hyphens at indices 4 and 7)
    if (dateStr.length === 10 && dateStr.charCodeAt(4) === 45 && dateStr.charCodeAt(7) === 45) {
        ...
        // Ensure all extracted characters are digits (ASCII 48-57)
        // to prevent malformed inputs from computing valid values
