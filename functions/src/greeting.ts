...
export function buildPrompt(request: GreetingRequest): string {
    ...
    const name = sanitizeInput(request.name);
    const leaderName = request.leaderName ? sanitizeInput(request.leaderName) : '';

    const leaderMention = leaderName ? ` on behalf of <leader_name>${leaderName}</leader_name>` : '';

    return `<instruction>
Generate a warm and heartfelt ${occasion} greeting message${leaderMention} for <recipient_name>${name}</recipient_name> in ${language}.
