...
function buildPrompt(request: GreetingRequest): string {
    ...
    // Sanitize user inputs to prevent prompt injection, fallback to generic terms if stripped entirely
    const sanitizedName = sanitizeInput(request.name) || 'the constituent';
    let sanitizedLeader = '';
    if (request.leaderName) {
        sanitizedLeader = sanitizeInput(request.leaderName) || 'the leader';
    }
    const leaderMention = sanitizedLeader ? ` on behalf of ${sanitizedLeader}` : '';
...
function getTemplateMessage(request: GreetingRequest): string {
    ...
    // Use sanitized name to prevent XSS in the generated template
    const sanitizedName = sanitizeInput(request.name) || 'the constituent';
