...
const VALID_TYPES: TaskType[] = ['BIRTHDAY', 'ANNIVERSARY'];
const VALID_LANGUAGES: Language[] = ['ODIA', 'ENGLISH', 'HINDI'];
 ...
 */
function getTemplateMessage(request: GreetingRequest): string {
...
export async function generateGreetingMessage(
    ...
    // Sanitize inputs to prevent prompt injection and XSS
    const sanitizedName = sanitizeInput(request.name);
    // If sanitization completely strips the name, use a safe fallback
    const safeName = sanitizedName || 'the constituent';

    const sanitizedLeaderName = request.leaderName ? sanitizeInput(request.leaderName) : undefined;
    const safeLeaderName = sanitizedLeaderName || (request.leaderName ? 'the leader' : undefined);

    const safeRequest: GreetingRequest = {
        ...request,
        name: safeName,
        leaderName: safeLeaderName,
    };
