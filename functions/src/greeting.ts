...
function buildPrompt(request: GreetingRequest): string {
    ...
    // If name is completely sanitized away (e.g. was just HTML tags), fallback to avoid empty names in prompts
    const finalName = safeName.length > 0 ? safeName : 'the constituent';
    const safeLeaderName = sanitizeInput(request.leaderName);
    const leaderMention = safeLeaderName ? ` on behalf of ${safeLeaderName}` : '';

    return `Generate a warm and heartfelt ${occasion} greeting message${leaderMention} for ${finalName} in ${language}.
