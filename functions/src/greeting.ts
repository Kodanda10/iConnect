...
export async function generateGreetingMessage(
    ...
    // If sanitization strips all content, fallback to a safe default to maintain grammatical structure
    const safeName = sanitizedName || 'the constituent';
    const safeLeaderName = request.leaderName ? sanitizeInput(request.leaderName) : undefined;
