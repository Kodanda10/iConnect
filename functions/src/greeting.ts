...
function buildPrompt(request: GreetingRequest): string {
    ...
    // Sanitize user inputs and fallback if malicious/empty
    const sanitizedName = sanitizeInput(request.name) || 'the constituent';
    const sanitizedLeaderName = request.leaderName ? sanitizeInput(request.leaderName) : '';
...
function getTemplateMessage(request: GreetingRequest): string {
    ...
    // Sanitize user input and fallback to prevent rendering empty strings
    const sanitizedName = sanitizeInput(request.name) || 'the constituent';
