...
export async function generateGreetingMessage(
    ...
    const safeName = sanitizedName ? sanitizedName : 'the constituent';

    const sanitizedLeader = sanitizeInput(request.leaderName);
    const safeLeader = sanitizedLeader ? sanitizedLeader : (request.leaderName ? 'the leader' : undefined);

    const safeRequest: GreetingRequest = {
        ...request,
        name: safeName,
        leaderName: safeLeader
    };
