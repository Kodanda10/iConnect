...
function buildPrompt(request: GreetingRequest): string {
    ...
    const cleanLeaderName = sanitizeInput(request.leaderName);
    const leaderMention = cleanLeaderName ? ` on behalf of ${cleanLeaderName}` : '';

    return `Generate a warm and heartfelt ${occasion} greeting message${leaderMention} for ${cleanName} in ${language}.
