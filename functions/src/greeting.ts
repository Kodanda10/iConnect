...
export async function generateGreetingMessage(
    ...
    // Note: Falling back to generic placeholders if the input is completely stripped
    // ensures the prompt maintains grammatical structure without reintroducing vulnerabilities.
    const sanitizedRequest: GreetingRequest = {
        ...request,
        name: sanitizeInput(request.name) || 'the constituent',
        leaderName: request.leaderName ? sanitizeInput(request.leaderName) : undefined,
    };
