...
function buildPrompt(request: GreetingRequest): string {
    ...
    const leaderMention = safeLeaderName ? ` on behalf of <sender>${safeLeaderName}</sender>` : '';

    return `<instruction>Generate a warm and heartfelt ${occasion} greeting message${leaderMention} for <recipient>${safeName}</recipient> in ${language}.
The message should be:
- Personal and sincere
- 2-3 sentences maximum
- Culturally appropriate for Indian context
- Include blessings for health and happiness
Only return the greeting message, nothing else.</instruction>`;
}
