...
export async function generateGreetingMessage(
    ...
    // Sanitize user inputs after validation to prevent prompt injection and XSS
    const safeRequest: GreetingRequest = {
        ...request,
        name: sanitizeInput(request.name) || 'Friend',
        ward: sanitizeInput(request.ward),
        leaderName: sanitizeInput(request.leaderName),
    };
