...
export function buildPrompt(request: GreetingRequest): string {
    ...
    const sName = sanitizeInput(request.name);
    const sLeader = sanitizeInput(request.leaderName);
    const leaderMention = sLeader ? ` on behalf of ${sLeader}` : '';

    return `Generate a warm and heartfelt ${occasion} greeting message${leaderMention} for <constituent_name>${sName}</constituent_name> in ${language}.
The message should be:
- Personal and sincere
- 2-3 sentences maximum
- Culturally appropriate for Indian context
- Include blessings for health and happiness
- Use the name provided in the <constituent_name> tag.
