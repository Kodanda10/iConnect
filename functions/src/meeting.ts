 ...
 */
export const createMeetingTicker = functions.region("asia-south1").https.onCall(async (request) => {
    ...
    if (!title || !startTime || !leaderUid || !meetingType) {
    ...
    try {
        ...
        throw new functions.https.HttpsError("internal", "Failed to create meeting ticker");
    }
});
 ...
 */
export const createConferenceBridge = functions.region("asia-south1").https.onCall(async (request) => {
